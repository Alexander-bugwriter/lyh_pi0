    ...
    def _load_spatial_features_index(self):
        ...
        if os.path.exists(index_cache_file):
            ...
            with open(index_cache_file, 'rb') as f:
                cache_data = pickle.load(f)
                self.spatial_files_index  = cache_data['spatial_path_index']
            print(f"索引缓存加载成功，共 {len(self.spatial_files_index)} 个episodes")
            ...
            for file_path in feature_files:
                ...
                self.spatial_files_index[episode_id] = file_path
        
            # 保存路径索引缓存
            with open(index_cache_file, 'wb') as f:
                pickle.dump({'spatial_path_index': self.spatial_files_index}, f)
