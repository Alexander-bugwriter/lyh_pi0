...
import sys
sys.path.insert(0, "/home/lyh_pi0/third_party/LIBERO")  # 手动添加路径

# 测试导入
try:
    import libero
    print(f"import succeeded libero: {libero.__file__}")
except ImportError:
    print("import failed,current path:", sys.path)
    raise

import os
os.environ["LEROBOT_DEVICE"] = "cuda" if torch.cuda.is_available() else "cpu"
DATASET_DIR = "/data/libero"
os.environ["LIBERO_DATASET_DIR"] = DATASET_DIR
PATH_TO_PI_MODEL = (
    "/home/pi0_model_checkpoint/pytorch/pi0_libero"
)
PATH_TO_JAX_PI_MODEL = "/home/pi0_model_checkpoint/jax/pi0_libero"
