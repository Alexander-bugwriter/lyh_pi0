...
from .history_buffer_key import UnlimitedHistoryBuffer
...
class PaliGemmaWithExpertModel(PreTrainedModel):
    ...
    def __init__(self, config: PaliGemmaWithExpertConfig):
        ...
        # )
        self.spatial_separator_token = TemporalSeparatorToken(
            hidden_dim=config.paligemma_config.projection_dim,
            num_history_frames=config.num_sampled_history_frames  # 就是3
        )
    ...
    def save_modular_components(self, save_path=None):
        ...
        torch.save(self.spatial_separator_token.state_dict(), save_path / 'spatial_separator_token.pth')
    ...
    def embed_image(self, image: torch.Tensor):
        ...
        for i, camera_key in enumerate(IMAGE_KEYS):
            if spatial_mask[i]:
                # 用 spatial 处理的
                current_enhanced = spatial_enhanced[spatial_idx:spatial_idx+1]
                spatial_idx += 1
            else:
                # 标准处理
        ...
        for img_idx in range(num_images):
            ...
            if needs_spatial and camera_key in ["base_0_rgb", "left_wrist_0_rgb"]:
                ...
                if camera_key == "base_0_rgb":
                    spatial_tokens = {
                        'camera_tokens': precomputed_spatial_features['base_camera_tokens'].to(
                            device=raw_vision_features.device, dtype=raw_vision_features.dtype).squeeze(1),
                        'patch_tokens': precomputed_spatial_features['base_patch_tokens'].to(
                            device=raw_vision_features.device, dtype=raw_vision_features.dtype).squeeze(1)
                    }
                else:  # wrist cameras
                    spatial_tokens = {
                        'camera_tokens': precomputed_spatial_features['wrist_camera_tokens'].to(
                            device=raw_vision_features.device, dtype=raw_vision_features.dtype).squeeze(1),
                        'patch_tokens': precomputed_spatial_features['wrist_patch_tokens'].to(
                            device=raw_vision_features.device, dtype=raw_vision_features.dtype).squeeze(1)
                    }
            ...
            # === 2. 🔥 统一的历史特征拼接处理 ===
            if needs_history:
                final_enhanced = self._process_history_features(
                    current_enhanced=current_enhanced,
                    camera_key=camera_key,
    ...
    def _process_history_features(self, current_enhanced, camera_key, needs_spatial, 
                                history_info_batch, batch_size):
        """
        🔥 新增辅助函数：统一处理历史特征拼接（支持 base 和 wrist）
        
        Args:
            current_enhanced: 当前帧的增强特征
            camera_key: 相机名称 ("base_0_rgb" 或 "left_wrist_0_rgb")
            needs_spatial: 是否使用spatial增强
            history_info_batch: 批次的历史信息
            batch_size: 批次大小
        
        Returns:
            final_enhanced: 拼接历史后的特征
        """
        batch_results = []
        ...
        for batch_idx in range(batch_size):
            ...
            if (batch_idx < len(history_info_batch) and 
                ...
                for hist_frame in available_history:
                    # 根据相机类型选择对应的图像
                    if camera_key == "base_0_rgb":
                        hist_img_uint8 = hist_frame['base_image_uint8']
                        hist_camera_tokens_key = 'base_camera_tokens'
                        hist_patch_tokens_key = 'base_patch_tokens'
                    elif camera_key == "left_wrist_0_rgb":
                        # 使用wrist图像和tokens
                        hist_img_uint8 = hist_frame['wrist_image_uint8']
                        hist_camera_tokens_key = 'wrist_camera_tokens'
                        hist_patch_tokens_key = 'wrist_patch_tokens'
                    else:
                        continue  # 其他相机暂不支持
                    
                    # 归一化图像
                    hist_img = hist_img_uint8.to(device)
                    ...
                    if needs_spatial:
                        ...
                        # 使用预计算的spatial tokens
                        hist_spatial_tokens = {
                            'camera_tokens': hist_frame[hist_camera_tokens_key].to(device=device, dtype=dtype),
                            'patch_tokens': hist_frame[hist_patch_tokens_key].to(device=device, dtype=dtype)
                        }
            ...
            if history_features:
                sequence_parts = []
                num_history = len(history_features)  # 应该是3
                ...
                for i, hist_feat in enumerate(history_features):
                    ...
                    separator = self.spatial_separator_token(
                        time_step=time_step,
                        batch_size=1,
                        device=device,
                        dtype=dtype
                ...
                # 当前帧（不加分隔符）
                sequence_parts.append(current_sample)
                enhanced_sample = torch.cat(sequence_parts, dim=1)
            else:
                enhanced_sample = current_sample
...
class TemporalSeparatorToken(nn.Module):
    """时序感知的分隔符Token - 仅3个"""
    def __init__(self, hidden_dim, num_history_frames=3):
        super().__init__()
        # 基础可学习token
        embed_std = 1 / torch.sqrt(torch.tensor(hidden_dim, dtype=torch.float32))
        self.base_token = nn.Parameter(torch.randn(1, 1, hidden_dim) * embed_std)
        
        # 时序位置编码 - 只需要 num_history_frames 个
        self.temporal_embedding = nn.Embedding(num_history_frames, hidden_dim)
        
    def forward(self, time_step, batch_size=1, device=None, dtype=None):
        """
        Args:
            time_step: int, 1-indexed (1, 2, 3)
        Returns:
            [B, 1, D] 时序感知的分隔符
        """
        if device is None:
            device = self.base_token.device
        if dtype is None:
            dtype = self.base_token.dtype
            
        base = self.base_token.expand(batch_size, -1, -1).to(device=device, dtype=dtype)
        
        # time_step 是 1-indexed，转为 0-indexed
        time_idx = torch.tensor([time_step - 1], device=device).long()
        temporal_encoding = self.temporal_embedding(time_idx).unsqueeze(0)
        temporal_encoding = temporal_encoding.expand(batch_size, -1, -1).to(dtype=dtype)
        
        return base + temporal_encoding