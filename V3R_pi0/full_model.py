...
from .paligemma_with_2_expert import PaliGemmaWithExpertConfig, PaliGemmaWithExpertModel
from .utils import (
...
class PI0FlowMatching(nn.Module):
    ...
    def __init__(self, config,paligemma_with_expert_config):
        ...
        # Memory-Imagine Expert
        self.use_memory_imagine = getattr(config, 'use_memory_imagine_expert', False)
        if self.use_memory_imagine:
            from .memory_imagine_expert.builder import build_memory_imagine_expert
            
            memory_components = build_memory_imagine_expert(config, device=self.device)
            self.memory_imagine_pool = memory_components['feature_pool']
            self.memory_imagine_decoder = memory_components['decoder']
            self.memory_imagine_config = memory_components['config']
            
            print("Memory-Imagine Expert initialized")
    ...
    def forward(
        ...
        # 异步更新Memory-Imagine特征池
        if self.use_memory_imagine:
            self.memory_imagine_pool.update_async(prefix_embs, state)
    ...
    @torch.no_grad()
    def generate_4d_scene(self):
        """
        生成Memory-Imagine 4D场景
        
        Returns:
            dict: 包含4D场景和渲染结果的字典
        """
        if not self.use_memory_imagine:
            return None
        
        # 从特征池获取最新数据
        latest_data = self.memory_imagine_pool.get_latest()
        if latest_data is None:
            return None
        
        features, state, timestamp = latest_data
        
        # 生成4D场景
        occupancy_4d = self.memory_imagine_decoder(features, state)
        
        return {
            'occupancy_4d': occupancy_4d,
            'memory_frames': occupancy_4d[:, :self.memory_imagine_config.memory_frames],
            'current_frame': occupancy_4d[:, self.memory_imagine_config.memory_frames:self.memory_imagine_config.memory_frames+1],
            'imagine_frames': occupancy_4d[:, self.memory_imagine_config.memory_frames+1:],
            'timestamp': timestamp,
            'config': self.memory_imagine_config
        }
